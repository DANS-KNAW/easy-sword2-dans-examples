...
public class SequenceSimpleDeposit {
  ...
  public static void main(String[] args) throws Exception {
    ...
    File zipFile = new File(baseBagDir.getAbsolutePath() + ".zip");
    Common.zipDirectory(baseBagDir, zipFile);
    URI baseUri = SimpleDeposit.depositPackage(zipFile, colIri, uid, pw);

    for(int i = 1; i < bagNames.length; ++i) {
      File bagDir = new File(bagNames[i]);
      Common.setBagIsVersionOf(bagDir, baseUri);
      zipFile = new File(bagDir.getAbsolutePath() + ".zip");
      Common.zipDirectory(bagDir, zipFile);
      SimpleDeposit.depositPackage(zipFile, colIri, uid, pw);
    }
  }
}
